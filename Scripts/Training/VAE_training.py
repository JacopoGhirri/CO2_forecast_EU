...
from Scripts.Elements.Models import Encoder, Decoder, VAEModel, VAE_loss_function
from Scripts.utils import load_config, load_datasets, save_datasets, check_NaN_grad, init_weights
...
def main():
    ...
    if os.path.exists(dataset_filename):
        ...
        full_dataset = Dataset_unified(path_csvs='Data/full_timeseries/',
                                       output_configs=output_configs,
                                       select_years=np.arange(2010, 2023 + 1),
                                       select_geo=['AT', 'BE', 'BG', 'HR', 'CY', 'CZ', 'DK', 'EE', 'EL', 'FI', 'FR',
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=False)  # , num_workers=16)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, drop_last=False)

    print("Data setup done")
    input_dim = len(full_dataset.input_variable_names)

    enc = Encoder(input_dim=input_dim, latent_dim=vae_latent_dim, num_blocks=vae_num_blocks, dim_blocks=vae_dim_blocks,
                  activation=vae_activation, normalization=vae_normalization, dropouts=vae_dropouts,
                  input_dropout=vae_input_dropout).cuda()
    dec = Decoder(input_dim=input_dim, latent_dim=vae_latent_dim, num_blocks=vae_num_blocks, dim_blocks=vae_dim_blocks,
                  activation=vae_activation, normalization=vae_normalization, dropouts=vae_dropouts).cuda()
    ...
    if vae_optimizer == 'adamw':
        optimizer = torch.optim.AdamW(vae_model.parameters(), lr=vae_learning_rate,
                                      weight_decay=vae_weight_decay, eps=1e-6)  # , betas=(beta1, beta2))
    elif vae_optimizer == 'adam':
        optimizer = torch.optim.Adam(vae_model.parameters(), lr=vae_learning_rate,
                                     weight_decay=vae_weight_decay, eps=1e-6)
    elif vae_optimizer == 'radam':
        optimizer = torch.optim.RAdam(vae_model.parameters(), lr=vae_learning_rate,
                                      weight_decay=vae_weight_decay, eps=1e-6)
    ...
    for epoch in range(vae_num_epochs):
        ...
        for idx, sample in enumerate(train_loader):
            ...
            loss = vae_wr * temp_r + temp_d * vae_wd  # weight_KLD(e=epoch, a=alpha_KD, w=wd, me=num_epochs)
            loss.backward()
        ...
        with torch.inference_mode():
            for val_idx, val_sample in enumerate(val_loader):
                ...
                temp_r, temp_d = vae_criterion(val_input_current, val_predictions, val_means, val_variances)
