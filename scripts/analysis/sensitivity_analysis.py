"""
Unified sensitivity analysis for emission predictions.

This script performs both variance-based (Sobol) and correlation-based (Spearman)
sensitivity analyses on the same set of perturbed samples, ensuring that both
methods evaluate identical model runs for direct comparability.

Sobol total-order indices (ST) capture each variable's full contribution to output
variance including interactions.  Spearman rank correlations measure monotonic
...
Usage:
    python -m scripts.analysis.sensitivity.sensitivity_analysis

Outputs:
    For each mode (full/inputonly):
    - data/sensitivity/sobol_results_{mode}.csv
    - data/sensitivity/spearman_results_{mode}.csv

Reference:
    Section 4 "Methods" discusses sensitivity analysis methodology.
    Figure 4 panels (a) and (b) show Sobol and Spearman results respectively.
"""
...
def run_sensitivity(
    ...
    """
    Run unified sensitivity analysis (Sobol + Spearman) for all sectors.

    Computes total-order Sobol indices (ST) and Spearman rank correlations
    on the same Saltelli sample set, so both methods see identical model runs.
    ...
    Returns:
        Tuple of (problem_dict, sobol_results_dict, spearman_results_dict)
    """
    sobol_output_file = f"sobol_results_{mode}.csv"
    spearman_output_file = f"spearman_results_{mode}.csv"
    ...
    out_csv = OUTPUT_DIR / sobol_output_file
    ...
    print(f"Sobol results ({mode}) written to {out_csv}")

    # =========================================================================
    # Spearman correlation analysis (on the same samples)
    # =========================================================================
    spearman_results = {}
    out_csv_spearman = OUTPUT_DIR / spearman_output_file

    with open(out_csv_spearman, "w", newline="") as outf:
        writer = csv.writer(outf)
...
def main():
    """Main entry point for unified sensitivity analysis."""
    print("=" * 70)
    print("UNIFIED SENSITIVITY ANALYSIS (SOBOL + SPEARMAN)")
    print("=" * 70)
    ...
    print("Sensitivity indices: Total-order Sobol (ST) + Spearman rank correlation")
    ...
    for mode in RUN_MODES:
        ...
        # Run analysis
        run_sensitivity(var_specs, mode=mode)

    print("\n" + "=" * 70)
    print("Unified sensitivity analysis complete for all modes!")
    print("=" * 70)


def run_single_mode(mode: VariableMode) -> None:
    """
    Run unified sensitivity analysis for a single mode.
    ...
    var_specs = build_grouped_var_specs(input_names, context_names, mode=mode)
    print(f"Running sensitivity analysis ({mode}): {len(var_specs)} variable groups")
