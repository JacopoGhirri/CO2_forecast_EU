...
def build_color_grid(n_change: int, n_abs: int) -> np.ndarray:
    ...
    change_colors = [
        mc.to_hex(np.clip([np.interp(x, anchor_xs, src_rgb[:, ch])
...
def compute_pct_and_abs(dataset, population_df: pd.DataFrame):
    ...
    def agg_year(yr):
        return (
            df_mc[df_mc["year"] == yr].groupby("geo")
            .agg(total=("total_CO2", "mean"),
                 **{s: (f"{s}_total", "mean") for s in OUTPUT_SECTORS})
            .add_suffix(f"_{yr}").reset_index()
        )
...
def draw_panel(ax, gdf, pct_col, abs_col, color_grid,
               change_thresh, abs_thresh, title, letter):
    ...
    with warnings.catch_warnings():
        ...
        for color, group in gdf.groupby("_color", sort=False):
            group.plot(ax=ax, color=color,
                       edgecolor=GEO_EDGE_COLOR, linewidth=GEO_EDGE_WIDTH)
...
def draw_legend(fig, rect, color_grid, change_thresh, abs_thresh,
                n_change, n_abs):
    """
    ...
    for i, t in enumerate(boundaries):
        ax.text(i + 1, -0.15, fmt_pct(t),
                ha="center", va="top", fontsize=FONT_LEGEND, color="#333333")
    # outer labels
    ax.text(0.5, -0.15, f"< {fmt_pct(change_thresh[0])}",
            ha="center", va="top", fontsize=FONT_LEGEND - 0.5, color="#666666")
    ax.text(n_change - 0.5, -0.15, f"> {fmt_pct(change_thresh[-1])}",
            ha="center", va="top", fontsize=FONT_LEGEND - 0.5, color="#666666")
    ...
    for ai, lbl in enumerate(ylabels):
        ax.text(-0.08, ai + 0.5, lbl, ha="right", va="center",
                fontsize=FONT_LEGEND, color="#333333")

    # Axis titles
    ax.text(n_change / 2, -0.85, "Change 2024–2030 (%)",
            ha="center", va="top", fontsize=FONT_LEGEND + 0.5, color="#333333")
    ax.text(-0.65, n_abs / 2, "MtCO2\n(2024)",
            ha="center", va="center", fontsize=FONT_LEGEND + 0.5,
...
def create_figure(pct_df, abs_df, eu_gdf):
    ...
    abs_r  = abs_df.rename(
        columns={c: f"{c}_abs" for c in abs_df.columns if c != "geo"})
    ...
    outer = gridspec.GridSpec(
        1, 2, figure=fig,
        left=0.01, right=0.99, top=0.97, bottom=0.28,
        wspace=0.04, width_ratios=[1, 2.2],
    )
    ax_total = fig.add_subplot(outer[0])
    right_gs = gridspec.GridSpecFromSubplotSpec(
        2, 3, subplot_spec=outer[1], hspace=0.10, wspace=0.05,
    )
    sector_axes = [fig.add_subplot(right_gs[r, c])
                   for r in range(2) for c in range(3)]

    # ── Total panel ───────────────────────────────────────────────────────
    ct = change_thresholds_zero_anchored(pct_df["total"], N_CHANGE_BINS)
    at = quantile_thresholds(abs_df["total"], N_ABS_BINS)
    draw_panel(ax_total, eu_gdf, "total", "total_abs",
               color_grid, ct, at, r"Total CO$_2$ emissions", "a")
    ...
    for i, (sector, ax) in enumerate(zip(OUTPUT_SECTORS, sector_axes)):
        ...
        draw_panel(ax, eu_gdf, sector, f"{sector}_abs",
                   color_grid, ct_s, at_s,
                   SECTOR_LABELS[sector], "bcdefg"[i])
    ...
    for ax, (ct_s, at_s) in zip(all_axes, all_thresh):
        ...
        draw_legend(fig,
                    ...
                    change_thresh=ct_s, abs_thresh=at_s,
                    n_change=N_CHANGE_BINS, n_abs=N_ABS_BINS)
