...
BIVARIATE_COLORS = {
    (-1, 2): "#08519c",  (-1, 1): "#6baed6",  (-1, 0): "#bdd7e7",
    ( 0, 2): "#737373",  ( 0, 1): "#bdbdbd",  ( 0, 0): "#e0e0e0",
    ( 1, 2): "#a50f15",  ( 1, 1): "#fb6a4a",  ( 1, 0): "#fcbba1",
}
...
def compute_sector_data(dataset, population_df):
    ...
    for geo in EU27_COUNTRIES:
        ...
        for s in OUTPUT_SECTORS:
            ...
            records.append({
                "geo": geo, "sector": s,
                "pct_change": pct, "mean_uncertainty": mean_unc,
                "emissions_2024": e2024, "emissions_2030": e2030_mean,
    ...
    for s in OUTPUT_SECTORS:
        ...
        pct_eu = ((e2030_eu - e2024_eu) / abs(e2024_eu)) * 100 if abs(e2024_eu) > 0 else 0
        records.append({
            "geo": "EU27", "sector": s,
            "pct_change": pct_eu, "mean_uncertainty": np.nan,
            "emissions_2024": e2024_eu, "emissions_2030": e2030_eu,
    ...
    for s in OUTPUT_SECTORS:
        ...
        if population_weighted:
            ...
            eu_unc[s] = (
                ...
                if total_pop > 0 else sec["mean_uncertainty"].mean()
            )
        else:
...
def create_heatmap(changes_df, change_thresholds, confidence_cutoffs):
    ...
    unc_pivot = changes_df.pivot(index="sector", columns="geo", values="mean_uncertainty")
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            ...
            if pd.notna(val):
                ...
                ax.text(j, i, fmt,
                        ha="center", va="center",
                        fontsize=size, color=txt_color, fontweight=weight)
    ...
    ax.set_xticklabels(
        ...
        rotation=45, ha="left", fontsize=15,
    )
    ...
    ax.set_title(
        ...
        fontsize=20, pad=8,          # tight gap between title and country labels
...
def main():
    ...
    all_unc = changes_df[changes_df["geo"].isin(EU27_COUNTRIES)]["mean_uncertainty"].dropna()
    confidence_cutoffs = [
