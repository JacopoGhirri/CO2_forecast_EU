"""
Figure 2 (Revised): Sectoral Emission Changes Heatmap in Absolute Mt CO2.

Shows change in emissions by sector in MEGATONNES (Mt CO2) rather than
percentage, so that large-emitter countries and sectors dominate visually.

Usage:
    python figure_sectoral_changes_revised.py

Changes from original:
    - Heatmap cells now show Mt CO2 change (2023 -> 2030) instead of %
    - Colorbar relabelled to Mt CO2
    - Stacked bar chart also in Mt CO2
"""
...
def compute_sector_changes(dataset, population_df):
    """Compute ABSOLUTE Mt CO2 change by sector from 2023 to 2030."""
    keys = dataset.keys
    ...
    for geo in EU27_COUNTRIES:
        ...
        for s in OUTPUT_SECTORS:
            ...
            # ---- KEY CHANGE: store absolute Mt instead of percentage ----
            delta_mt = (e2030_mean - e2023) / 1e6  # tonnes -> Mt
            sector_changes.append({
                "geo": geo,
                "sector": s,
                "emissions_2023_tonnes": e2023,
                "emissions_2030_tonnes": e2030_mean,
                "delta_Mt": delta_mt,
            })
    ...
    for s in OUTPUT_SECTORS:
        ...
        eu_by_mc = df_2030[df_2030["geo"].isin(EU27_COUNTRIES)].groupby("mc_sample")[
            f"{s}_total"
        ].sum()
        e2030_mean = eu_by_mc.mean()
        delta_mt = (e2030_mean - e2023) / 1e6
        sector_changes.append({
            "geo": "EU27",
            "sector": s,
            "emissions_2023_tonnes": e2023,
...
def create_heatmap(changes_df):
    """Heatmap with cells showing Mt CO2 change."""
    setup_nature_style()
    ...
    heatmap_data = changes_df.pivot(index="geo", columns="sector", values="delta_Mt")

    # Overall column (sum across sectors)
    overall = []
    for geo in heatmap_data.index:
        overall.append({"geo": geo, "Overall": heatmap_data.loc[geo].sum()})
    overall_df = pd.DataFrame(overall).set_index("geo")
    ...
    for i in range(len(row_order)):
        for j in range(len(column_order)):
            ...
            if pd.notna(val):
                ...
                fmt = f"{val:.1f}" if abs(val) < 10 else f"{val:.0f}"
    ...
    cbar.set_label("Change in emissions, 2023–2030 (Mt CO₂)", fontsize=7, labelpad=4)
    ...
    for fmt in ["png", "pdf", "svg"]:
        plt.savefig(OUTPUT_DIR / f"fig2_sectoral_change_heatmap_Mt.{fmt}",
                    dpi=300, bbox_inches="tight", facecolor="white")
    plt.close()
    print(f"Saved: {OUTPUT_DIR}/fig2_sectoral_change_heatmap_Mt.[png|pdf|svg]")


def create_stacked_bar(changes_df):
    """Stacked bar chart in Mt CO2."""
    setup_nature_style()
    pivot = changes_df.pivot(index="geo", columns="sector", values="delta_Mt")
    ...
    ax.axvspan(-500, 0, color="#eafaf1", alpha=0.7, zorder=0)
    ax.axvspan(0, 200, color="#fdedec", alpha=0.7, zorder=0)
    ...
    for i, geo in enumerate(country_order):
        ...
        for sector in OUTPUT_SECTORS:
            ...
            if val < 0:
                ax.barh(i, val, height=bar_height, left=left_neg, color=color,
                        edgecolor="white", linewidth=0.3, zorder=3)
                left_neg += val
            else:
                ax.barh(i, val, height=bar_height, left=left_pos, color=color,
                        edgecolor="white", linewidth=0.3, zorder=3)
                left_pos += val
    ...
    ax.set_xlabel("Cumulative change in emissions 2023→2030 (Mt CO₂)")
    ax.xaxis.grid(True, linestyle="-", alpha=0.2, color="#bdc3c7", linewidth=0.3)
    ax.set_axisbelow(True)
    ax.tick_params(axis="y", length=0)

    legend_elements = [
        mpatches.Patch(facecolor=SECTOR_CONFIG[s]["color"], edgecolor="white",
                       linewidth=0.3, label=SECTOR_CONFIG[s]["name"])
        for s in OUTPUT_SECTORS
    ]
    leg = ax.legend(handles=legend_elements, loc="lower right", frameon=True,
                    fontsize=6, framealpha=0.95, edgecolor="#bdc3c7",
                    handletextpad=0.4, labelspacing=0.35, borderpad=0.5,
                    title="Sector", title_fontsize=7)
    ...
    for fmt in ["png", "pdf", "svg"]:
        plt.savefig(OUTPUT_DIR / f"fig2_sectoral_change_stacked_bar_Mt.{fmt}",
                    dpi=300, bbox_inches="tight", facecolor="white")
    plt.close()
    print(f"Saved: {OUTPUT_DIR}/fig2_sectoral_change_stacked_bar_Mt.[png|pdf|svg]")


def main():
    print("=" * 70)
    print("GENERATING FIGURE 2 (REVISED): SECTORAL CHANGES IN Mt CO2")
    print("=" * 70)
    ...
    changes_df["sector_name"] = changes_df["sector"].map(lambda x: SECTOR_CONFIG[x]["name"])
    changes_df.to_csv(TABLE_DIR / "fig2_sectoral_change_data_Mt.csv", index=False)
    print(f"Exported: {TABLE_DIR}/fig2_sectoral_change_data_Mt.csv")

    print("\n" + "=" * 70)
    print("SUMMARY (2023 → 2030, Mt CO2)")
    print("=" * 70)
    print(f"\n{'Sector':<20} {'EU27':>10} {'Mean':>10} {'Min':>10} {'Max':>10}")
    print("-" * 60)
    for sector in OUTPUT_SECTORS:
        sd = changes_df[changes_df["sector"] == sector]
        eu = sd[sd["geo"] == "EU27"]["delta_Mt"].values[0]
        cd = sd[sd["geo"] != "EU27"]["delta_Mt"].dropna()
        print(f"{SECTOR_CONFIG[sector]['name']:<20} {eu:>+9.1f} {cd.mean():>+9.1f} "
              f"{cd.min():>+9.1f} {cd.max():>+9.1f}")
    print("\nFigure 2 (revised) generation complete!")
